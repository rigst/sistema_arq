...
def excedeu_rate_limit_visitante(ip: str) -> bool:
    tentativas = cache.get(_rate_limit_key(ip), 0)
...
def limpar_dados_visitante(user):
    """Apaga todos os dados do tenant do visitante e, por fim, o próprio usuário,
    o grupo e a empresa temporária. Os dados de cada módulo são removidos pelos
    limpadores registrados em `core.visitante_cleanup`."""
    if not user or not getattr(user, "eh_visitante", False):
        return

    grupo = obter_grupo_empresa_usuario(user)
    if grupo is not None:
        limpar_dados_negocio(grupo)

    user.delete()

    if grupo is not None and grupo.name.startswith(VISITOR_GROUP_PREFIX):
