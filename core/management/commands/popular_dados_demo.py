    ...
    def handle(self, *args, **options):
        ...
        for descricao, valor in (
            ...
            CustoFixo.objects.update_or_create(
                empresa=grupo, descricao=descricao,
                defaults={"valor_mensal": Decimal(valor), "ativo": True, "criado_por": usuario},
            )
        for nome, percentual in (("Urgência", "20.00"), ("Alta complexidade", "15.00"), ("Cliente recorrente", "-8.00")):
            FatorPrecificacao.objects.update_or_create(
                empresa=grupo, nome=nome,
        ...
        projetos = {}
        dados_projetos = (
            ("Residência Alameda", "Marina e Lucas Almeida", "residencial", "ativo", "42000.00", 280, True),
            ("Café Horizonte — Loja Centro", "Café Horizonte Ltda.", "comercial", "ativo", "28500.00", 190, True),
            ("Centro Educacional Caminhos", "Instituto Caminhos", "institucional", "ativo", "68000.00", 420, False),
            ("Loteamento Parque Sul", "Incorporadora Parque Sul", "urbanismo", "pausado", "95000.00", 560, False),
        )
        for indice, (nome, cliente_nome, tipo, status, valor, horas, execucao) in enumerate(dados_projetos):
            projeto, _ = Projeto.objects.update_or_create(
                empresa=grupo, nome=nome,
                defaults={
                    "cliente": clientes[cliente_nome], "tipo": tipo, "status": status,
                    "valor_contratado": Decimal(valor), "horas_estimadas": Decimal(horas),
                    "tem_execucao": execucao, "cidade": "Porto Alegre", "uf": "RS",
            ...
            montar_fases(projeto, complementares=("comp_estrutural", "comp_eletrica", "comp_hidraulica"))
        ...
        for indice, tarefa in enumerate(fase_ativa.tarefas.order_by("ordem")):
            ...
            tarefa.status = "concluida" if indice == 0 else ("andamento" if indice == 1 else "aberta")
        ...
        Tarefa.objects.update_or_create(
            empresa=grupo, projeto=residencia, fase=fase_ativa, titulo="Revisar compatibilização da cozinha",
            defaults={
                "descricao": "Conferir arquitetura, elétrica e hidráulica antes da apresentação.",
                "responsavel": usuario, "prazo": hoje + timedelta(days=3),
                "horas_previstas": Decimal("4"), "status": "andamento", "criado_por": usuario,
            },
        )

        proposta, _ = Proposta.objects.update_or_create(
            empresa=grupo, titulo="Proposta — Centro Educacional Caminhos",
            defaults={
        ...
        itens_proposta = (
            (0, "Levantamento e diagnóstico", 32, "Visita técnica, levantamento cadastral e diagnóstico das necessidades."),
            (1, "Estudo preliminar", 64, "Partido arquitetônico, implantação, plantas e apresentação ao cliente."),
            (2, "Anteprojeto", 96, "Desenvolvimento das soluções aprovadas e compatibilização inicial."),
            (3, "Projeto executivo", 180, "Detalhamentos, especificações e documentação para execução."),
        )
        ...
        for ordem, descricao, horas, inclusoes in itens_proposta:
            ItemProposta.objects.update_or_create(
                empresa=grupo, proposta=proposta, descricao=descricao,
                defaults={
                    "ordem": ordem, "horas_estimadas": Decimal(horas), "inclusoes": inclusoes,
                    "valor": precificar_etapa(grupo, horas, hora_tecnica=Decimal("185.00"))["total"],
                },
            )

        contrato, _ = Contrato.objects.update_or_create(
            empresa=grupo, projeto=residencia, numero="DEMO-2026-001",
            defaults={
                "titulo": "Contrato de serviços — Residência Alameda",
                "valor_total": Decimal("42000.00"), "data_assinatura": hoje - timedelta(days=40),
                "status": "ativo", "parcelas_lancadas": True, "criado_por": usuario,
        ...
        conta, _ = ContaBancaria.objects.update_or_create(
            empresa=grupo, nome="Conta empresarial — demonstração",
            defaults={"saldo_inicial": Decimal("12500.00"), "pessoal": False},
        )
        for numero, (dias, paga) in enumerate(((-30, True), (0, True), (30, False), (60, False)), 1):
            valor = Decimal("10500.00")
            lancamento, _ = Lancamento.objects.update_or_create(
                empresa=grupo, descricao=f"Contrato DEMO-2026-001 — parcela {numero}",
                defaults={
                    "conta": conta, "tipo": "entrada", "projeto": residencia, "valor": valor,
                    "data": hoje + timedelta(days=dias), "status": "realizado" if paga else "previsto",
                    "criado_por": usuario,
                },
            )
            Parcela.objects.update_or_create(
                empresa=grupo, contrato=contrato, numero=numero,
                defaults={
                    "descricao": f"Parcela {numero}/4", "valor": valor,
                    "vencimento": hoje + timedelta(days=dias), "paga": paga,
        ...
        for descricao, tipo, valor, dias in extras:
            Lancamento.objects.update_or_create(
                empresa=grupo, descricao=descricao,
                defaults={
                    "conta": conta, "tipo": tipo, "projeto": None, "categoria": None,
                    "valor": Decimal(valor), "data": hoje + timedelta(days=dias),
                    "status": "realizado" if dias <= 0 else "previsto", "criado_por": usuario,
                },
            )
        ...
        for nome, categoria, contato in (
            ...
            fornecedores[nome], _ = Fornecedor.objects.update_or_create(
                empresa=grupo, nome=nome,
                defaults={
                    "categoria": categoria, "contato": contato, "telefone": "(51) 3333-2026",
                    "email": f"contato@{nome.lower().replace(' ', '')[:18]}.example",
                    "cidade": "Porto Alegre", "prazo_medio_dias": 18, "avaliacao": 5,
                    "ativo": True, "criado_por": usuario,
                },
            )

        cafe = projetos["Café Horizonte — Loja Centro"]
        orcamento, _ = Orcamento.objects.update_or_create(
            empresa=grupo, projeto=cafe, titulo="Orçamento de execução — Loja Centro",
            defaults={
                "versao": "1", "status": "rascunho", "bdi_percent": Decimal("12.00"),
                "validade": hoje + timedelta(days=20), "criado_por": usuario,
            },
        )
        for ordem, ambiente, categoria, descricao, unidade, quantidade, unitario, fornecedor_nome in (
            (0, "Salão", "marcenaria", "Balcão de atendimento sob medida", "un", 1, "18500.00", "Madeira Sul Marcenaria"),
            (1, "Fachada", "outro", "Letreiro e sinalização externa", "vb", 1, "6800.00", "Traço Comunicação Visual"),
            (2, "Salão", "instalacoes", "Adequações elétricas e luminotécnicas", "vb", 1, "12400.00", "Eixo Engenharia"),
        ):
            ItemOrcamento.objects.update_or_create(
                empresa=grupo, orcamento=orcamento, descricao=descricao,
                defaults={
                    "ordem": ordem, "ambiente": ambiente, "categoria": categoria,
                    "unidade": unidade, "quantidade": Decimal(quantidade),
                    "valor_unitario": Decimal(unitario), "fornecedor": fornecedores[fornecedor_nome],
                },
            )

        obra, criada = Obra.objects.get_or_create(
            empresa=grupo, projeto=residencia,
            defaults={
                "endereco": residencia.endereco, "responsavel_tecnico": "Arq. Rodrigo Stolben",
                "status": "andamento", "data_inicio": hoje - timedelta(days=25),
                "data_prevista_fim": hoje + timedelta(days=150), "criado_por": usuario,
            },
        )
        ...
        compromissos = (
            ("Apresentação — Centro Educacional", "apresentacao", 2, 14, clientes["Instituto Caminhos"], projetos["Centro Educacional Caminhos"]),
            ("Visita técnica — Residência Alameda", "visita", 4, 9, clientes["Marina e Lucas Almeida"], residencia),
            ("Reunião com engenharia — Café Horizonte", "reuniao", 7, 16, clientes["Café Horizonte Ltda."], cafe),
        )
        for titulo, tipo, dias, hora, cliente, projeto in compromissos:
            inicio = timezone.make_aware(datetime.combine(hoje + timedelta(days=dias), time(hora, 0)))
            Compromisso.objects.update_or_create(
                empresa=grupo, titulo=titulo,
                defaults={
                    "tipo": tipo, "inicio": inicio, "fim": inicio + timedelta(hours=1),
                    "local": "Escritório / videoconferência", "cliente": cliente,
                    "projeto": projeto, "criado_por": usuario,
                },
            )
        ...
        self.stdout.write(self.style.SUCCESS(f"Dados de demonstração disponíveis para {username}: {totais}"))
