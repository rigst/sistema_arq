...
def test_credencial_errada_nao_derruba_a_pagina(live_server, page):
    ...
    page.goto(f"{live_server.url}/login/")
    page.fill("form.auth-form input[type=password]", "senha-que-nao-existe")
...
def test_paginas_legais_nao_exigem_login(live_server, page):
    ...
    # lá. É a mesma razão de o job de a11y ter o a11y-setup-command.
    for tipo in (DocumentoLegal.TERMOS, DocumentoLegal.PRIVACIDADE):
        DocumentoLegal.objects.create(
            tipo=tipo,
            versao="1.0",
            titulo=f"Documento de teste ({tipo})",
            conteudo="Conteúdo mínimo para a página renderizar.",
            vigente_desde=timezone.now() - timezone.timedelta(days=1),
        )
